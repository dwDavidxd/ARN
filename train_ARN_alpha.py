...
while global_step < trainer_conf['total_step']:
    ...
    for nat, advu1, advu2, _ in data_loader:
        print('\repoch-' + str(math.floor(global_step / 200)) + ':' + str(global_step % 200), end='')
        ...
        if trainer_conf['save_log'] and (global_step % trainer_conf['verbose_step'] == 0):
            writer.add_scalar('MSE', loss_lambda['mse_for']['cur'] * loss_mse.data.item(), global_step)
            writer.add_scalar('Nor', loss_lambda['kl']['cur'] * kl.data.item(), global_step)
            writer.add_scalar('gp', gp.data.item(), global_step)
            writer.add_scalars('Pixel_Distance', {'real': pix_real_pred.data.item(),
                                                  'fake': pix_fake_pred.data.item(),
                                                  'adv': pixel_loss.data.item()}, global_step)
            writer.add_scalars('Code_loss', {'feature': df_loss.data.item(),
                                             'adv_feature': feature_loss.data.item()}, global_step)
        ...
        if global_step % trainer_conf['plot_step'] == 0:
            print('MSE loss: {}'.format(loss_lambda['mse_for'] * loss_mse.data.item()))
            print('Pixel adv_loss: {}'.format(pixel_loss.data.item()))
            print('Feature adv_loss: {}'.format(att_loss.data.item()))
            ae.eval()

            # Generate
            for nat, advu1, advu2, _ in data_loader_test:
                nat = nat.repeat(1, 3, 1, 1)
                advu1 = advu1.repeat(1, 3, 1, 1)
                advu2 = advu2.repeat(1, 3, 1, 1)
                input = torch.cat((nat, advu1, advu2), dim=0)
